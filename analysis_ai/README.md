...
- **Backups of its own data.** Settings can archive `data/` daily or weekly into `../backups` (`AAI_BACKUP_LOCATION`), the same folder the notes app backs up into, keeping the last N. Each archive is read back and checksummed before it counts, and retention only deletes archives made here — the notes app's dumps, and anything else in that folder, are never touched.
